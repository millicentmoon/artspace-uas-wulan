...
<head>
    ...
    <title>Dashboard - ArtSpace</title>
...
<body class="overflow-x-hidden">
        ...
        <div class="p-6">
                ...
                <div class="flex items-center gap-3 mb-3 border-b-2 border-blue-100 pb-2">
                    ...
                    <div>
                        <p class="text-xs text-gray-400 font-bold uppercase">
                            Halo, {{ Auth::user()->role === 'admin' ? 'Admin' : 'Member' }}
                        </p>
                        <h4 class="text-sm font-bold text-blue-600 truncate max-w-[150px]">{{ Auth::user()->name }}</h4>
                    </div>
                </div>
                
                <ul class="space-y-2 text-sm font-bold">
                    <li>
                        <a href="{{ route('dashboard') }}" class="flex items-center text-white bg-blue-500 hover:bg-blue-600 p-2 rounded-lg transition shadow-pop-blue">
                            <span class="mr-3">🏠</span> Dashboard Utama
                        </a>
                    </li>

                    @if(Auth::user()->role === 'admin')
                    <li class="mt-4 pt-4 border-t border-blue-200">
                        <span class="text-xs text-gray-400 uppercase tracking-widest px-2 mb-2 block">Menu Admin</span>
                    </li>
                    <li>
                        <a href="{{ route('articles.index') }}" class="flex items-center text-purple-600 bg-purple-100 hover:bg-purple-200 p-2 rounded-lg transition">
                            <span class="mr-3">⚙️</span> Kelola Artikel
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('articles.create') }}" class="flex items-center text-green-600 bg-green-100 hover:bg-green-200 p-2 rounded-lg transition">
                            <span class="mr-3">➕</span> Tambah Artikel
                        </a>
                    </li>
                    @endif
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 gap-2">
                    <a href="{{ route('dashboard', ['category' => 'Seni Kuno']) }}" class="bg-yellow-100 text-yellow-800 px-3 py-2 rounded-lg font-bold hover:bg-yellow-200 transition text-sm">🏺 Seni Kuno</a>
                    <a href="{{ route('dashboard', ['category' => 'Seni Prasejarah']) }}" class="bg-orange-100 text-orange-800 px-3 py-2 rounded-lg font-bold hover:bg-orange-200 transition text-sm">🦖 Seni Prasejarah</a>
                    <a href="{{ route('dashboard', ['category' => 'Abad Pertengahan']) }}" class="bg-purple-100 text-purple-800 px-3 py-2 rounded-lg font-bold hover:bg-purple-200 transition text-sm">🏰 Abad Pertengahan</a>
                    <a href="{{ route('dashboard', ['category' => 'Modern']) }}" class="bg-blue-100 text-blue-800 px-3 py-2 rounded-lg font-bold hover:bg-blue-200 transition text-sm">🏙️ Seni Modern</a>
                </div>
            </div>
            ...
            <div>
                ...
                <ul class="space-y-2 font-bold text-sm text-gray-600">
                    <li><a href="{{ route('dashboard', ['category' => 'Lukisan']) }}" class="block hover:text-pink-500 transition border-l-4 border-transparent hover:border-pink-500 pl-2">🎨 Lukisan</a></li>
                    <li><a href="{{ route('dashboard', ['category' => 'Patung']) }}" class="block hover:text-pink-500 transition border-l-4 border-transparent hover:border-pink-500 pl-2">🗿 Patung</a></li>
                    <li><a href="{{ route('dashboard', ['category' => 'Arsitektur']) }}" class="block hover:text-pink-500 transition border-l-4 border-transparent hover:border-pink-500 pl-2">🏛️ Arsitektur</a></li>
                    <li><a href="{{ route('dashboard', ['category' => 'Fotografi']) }}" class="block hover:text-pink-500 transition border-l-4 border-transparent hover:border-pink-500 pl-2">📸 Fotografi</a></li>
                    <li><a href="{{ route('dashboard', ['category' => 'Desain Digital']) }}" class="block hover:text-pink-500 transition border-l-4 border-transparent hover:border-pink-500 pl-2">💻 Desain Digital</a></li>
                </ul>
            </div>
        </div>
    </aside>
    ...
    <nav class="bg-white border-b-4 border-blue-300 py-4 sticky top-0 z-30 shadow-sm">
        <div class="max-w-7xl mx-auto px-6 flex justify-between items-center">
            <div class="flex items-center gap-4">
                ...
                <form action="{{ route('dashboard') }}" method="GET" class="hidden md:block">
                    <input type="text" name="search" value="{{ request('search') }}" class="bg-gray-100 border-2 border-gray-200 text-sm text-gray-700 rounded-full px-4 py-2 w-64 focus:outline-none focus:border-pink-400 focus:bg-white transition" placeholder="Cari artikel...">
                </form>
            </div>
            <div class="flex items-center gap-3">
                <div class="hidden lg:flex items-center gap-2 text-sm font-bold text-gray-600 bg-gray-100 px-3 py-1 rounded-full border border-gray-200">
    ...
    <main class="max-w-7xl mx-auto px-6 py-12 min-h-screen">
        ...
        @if(session('success'))
            <div class="bg-green-100 border-l-4 border-green-500 text-green-700 p-4 mb-8 rounded shadow-sm text-center font-bold animate-pulse">
                {{ session('success') }}
            </div>
        @endif
        ...
        @if(isset($singlePost) && $singlePost)
            <div class="max-w-4xl mx-auto animate-fade-in-up">
                <div class="bg-white p-8 rounded-[2rem] border-4 border-blue-200 shadow-pop-pink relative group">
                    <span class="absolute top-0 left-0 bg-yellow-300 text-yellow-900 px-4 py-1 rounded-br-xl rounded-tl-xl font-bold font-ceria z-10">✨ Mode Baca</span>
                    
                    <div class="absolute top-4 right-4 z-20 flex gap-2">
                        @if(Auth::user()->role === 'admin')
                        <form action="{{ route('articles.destroy', $singlePost->id) }}" method="POST" onsubmit="return confirm('Yakin hapus?');">
                            @csrf @method('DELETE')
                            <button class="bg-red-100 text-red-500 px-3 py-2 rounded-lg font-bold hover:bg-red-200 transition">🗑️</button>
                        </form>
                        <a href="{{ route('articles.edit', $singlePost->id) }}" class="bg-white border-2 border-gray-800 text-gray-800 px-4 py-2 rounded-lg font-bold shadow-pop-yellow hover:bg-yellow-300 hover:text-black transition flex items-center gap-2 transform hover:rotate-2">
                            <span>✏️</span> EDIT
                        </a>
                        @endif
                    </div>
                    ...
                    <div class="overflow-hidden rounded-xl h-96 mb-6 border-2 border-gray-100 shadow-inner bg-gray-100">
                        @if($singlePost->image) <img src="{{ asset('storage/' . $singlePost->image) }}" class="w-full h-full object-cover">
                        @else <img src="https://picsum.photos/seed/{{ $singlePost->id }}/800/400" class="w-full h-full object-cover"> @endif
                    </div>
                    <article class="prose lg:prose-xl text-gray-600 line-clamp-3">
                        {{ $singlePost->content }}
                    </article>
                    <div class="mt-8 text-center">
                        <a href="{{ route('visitor.read', $singlePost->id) }}" class="inline-block bg-blue-500 text-white px-6 py-2 rounded-full font-bold hover:bg-blue-600 transition shadow-lg">📖 Baca Selengkapnya</a>
                        <a href="{{ route('dashboard') }}" class="inline-block bg-gray-100 text-gray-500 px-6 py-2 rounded-full font-bold hover:bg-pink-100 hover:text-pink-500 transition ml-2">Tutup</a>
                    </div>
                </div>
            </div>
        ...
        @elseif(isset($searchMessage))
            <div class="text-center py-10 mb-10">
                ...
                <div class="mt-4">
                    @if(Auth::user()->role === 'admin')
                    <a href="{{ route('articles.create') }}" class="inline-block bg-blue-500 text-white px-6 py-2 rounded-full font-bold hover:bg-blue-600 transition">Buat Artikel Baru</a>
                    @endif
                </div>
            </div>
        ...
        @else
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-20 items-start">
                ...
                <div class="lg:col-span-3 space-y-6">
                    <h3 class="font-ceria text-xl text-blue-500 border-b-2 border-dashed border-blue-200 pb-2 mb-4">🔥 Pilihan Editor</h3>
                    @foreach($others->take(2) as $item)
                    <a href="{{ Auth::user()->role === 'admin' ? route('articles.edit', $item->id) : route('visitor.read', $item->id) }}" class="block group cursor-pointer bg-white p-3 rounded-2xl shadow-sm hover:shadow-pop-blue border border-gray-100 transition duration-300">
                        ...
                        <h3 class="text-md font-ceria font-bold leading-tight text-gray-800 group-hover:text-pink-500 transition mt-2">✏️ {{ $item->title }}</h3>
                    </a>
                    @endforeach
                </div>

                <div class="lg:col-span-6">
                    <a href="{{ Auth::user()->role === 'admin' ? route('articles.edit', $featured->id) : route('visitor.read', $featured->id) }}" class="block group relative bg-white p-4 rounded-[2rem] border-4 border-pink-200 shadow-pop-pink hover:border-pink-400 transition duration-300">
                        <div class="overflow-hidden h-[450px] rounded-[1.5rem] relative bg-gray-200">
                            @if($featured->image) <img src="{{ asset('storage/' . $featured->image) }}" class="w-full h-full object-cover transition duration-700 transform group-hover:scale-105">
                            @else <img src="https://picsum.photos/seed/{{ $featured->id }}/800/1000" class="w-full h-full object-cover transition duration-700 transform group-hover:scale-105"> @endif
                            
                            @if(Auth::user()->role === 'admin')
                            <div class="absolute top-4 right-4 flex flex-col gap-2">
                                <span class="bg-white text-gray-800 p-3 rounded-xl shadow-lg border-2 border-yellow-300 hover:bg-yellow-300 transition transform hover:rotate-3 font-bold text-center">✏️ EDIT</span>
                            </div>
                            @endif
                        </div>
                        <div class="mt-6 text-center px-4 pb-4">
                            <span class="bg-pink-100 text-pink-600 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest">🌟 Artikel Terbarumu</span>
                            <h2 class="text-4xl font-ceria text-gray-900 mt-4 mb-3 leading-tight">{{ $featured->title }}</h2>
                            <p class="text-gray-500 leading-relaxed mb-6 line-clamp-3">{{ $featured->content }}</p>
                        </div>
                    </a>
                </div>

                <div class="lg:col-span-3 space-y-6">
                    <h3 class="font-ceria text-xl text-yellow-500 border-b-2 border-dashed border-yellow-200 pb-2 mb-4">🆕 Arsip Lain</h3>
            ...
            <div class="border-t-4 border-dotted border-pink-200 pt-12 mt-12">
                <h3 class="font-ceria text-2xl text-gray-800 border-b-2 border-dashed border-gray-300 pb-2 mb-8 text-center">📚 Koleksi Lainnya</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                    @foreach($others->skip(4) as $item)
                    <a href="{{ Auth::user()->role === 'admin' ? route('articles.edit', $item->id) : route('visitor.read', $item->id) }}" class="group cursor-pointer bg-white p-3 rounded-2xl shadow-sm hover:shadow-pop-blue border border-gray-100 transition duration-300">
                        <div class="overflow-hidden mb-3 h-32 rounded-xl bg-gray-100 relative">
                            @if($item->image) <img src="{{ asset('storage/' . $item->image) }}" class="w-full h-full object-cover transform group-hover:scale-110 transition duration-500">
                            @else <img src="https://picsum.photos/seed/{{ $item->id }}/400/300" class="w-full h-full object-cover transform group-hover:scale-110 transition duration-500"> @endif
                        </div>
                        <h3 class="text-md font-ceria font-bold leading-tight text-gray-800 group-hover:text-pink-500 transition mt-2 line-clamp-2">{{ $item->title }}</h3>
                    </a>
                    @endforeach
                </div>
            </div>
            ...
            <div class="text-center py-20 bg-white rounded-3xl border-2 border-dashed border-gray-300">
                <div class="text-6xl mb-4">📂</div>
                <h3 class="text-xl font-bold text-gray-400">Belum ada artikel.</h3>
                @if(Auth::user()->role === 'admin')
                <a href="{{ route('articles.create') }}" class="text-blue-500 font-bold hover:underline mt-2 inline-block">Buat Sekarang</a>
                @endif
            </div>
            @endif
        @endif
